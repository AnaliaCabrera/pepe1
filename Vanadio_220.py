                    ...
                    for n in range(col,0,-1):
                        ...
                        if n%2 == 0 :
                            ...
                            for kimp in range(aux1b-1,aux1a-1,-1) :
                                ...
                                Mo[kio][kj]=Mo[kio][kj]+peso*monitor[r+numors*25]
                                Mo[kio][kj+1]=Mo[kio][kj+1]+(1-peso)*monitor[r+numors*25]
                                
                                                               
                                if monitor[r+numors*25] != 0 and  f[kimp] != 0 :
                                    #Para la parte [2]
                                    Idrh2[128-n][kio][kj]=Idrh2[n-1][kio][kj]+peso*f[kimp]/monitor[r+numors*25]
                                    Idrh2[128-n][kio][kj+1]=Idrh2[n-1][kio][kj+1]+(1-peso)*f[kimp]/monitor[r+numors*25]
                                    
                                    Idrh1[128-n][kio][kj]=Idrh1[n-1][kio][kj]+peso*f[kimp]
                                    Idrh1[128-n][kio][kj+1]=Idrh1[n-1][kio][kj+1]+(1-peso)*f[kimp]
                ...
                #                I_2_d[kio][kj]=I_2_d[kio][kj]+peso*(f[kimp])**2
                 #               I_2_d[kio][kj+1]=I_2_d[kio][kj+1]+(1.0-peso)*(f[kimp])**2
                            ...
                            for k in range(aux1b-1,aux1a-1,-1) :
                                ...
                                Mo[ki][kj]=Mo[ki][kj]+peso*monitor[r+numors*25]
                                Mo[ki][kj+1]=Mo[ki][kj+1]+(1-peso)*monitor[r+numors*25]
                                
                                if monitor[r+numors*25] != 0 and  f[k] != 0 :
                                    #Para la parte [2]
                                    Idrh2[128-n][ki][kj]=Idrh2[n-1][ki][kj]+peso*f[k]/monitor[r+numors*25]
                                    Idrh2[128-n][ki][kj+1]=Idrh2[n-1][ki][kj+1]+(1-peso)*f[k]/monitor[r+numors*25]
                                    
                                    Idrh1[128-n][ki][kj]=Idrh1[n-1][ki][kj]+peso*f[k]
                                    Idrh1[128-n][ki][kj+1]=Idrh1[n-1][ki][kj+1]+(1-peso)*f[k]
...
W = np.sqrt(E_Idrh1)
W_relativo=np.divide(Idrh1,W,out=np.zeros_like(Idrh1),where=W!=0)#nuevo
    

Ideal, Ideal1 = Prom_Ponderado(Idrh2,W_relativo,fil,fil,col*25+1+maxteta-25)
#Mo=np.sum(Idrh2,axis=0)
Ideal_Mo=np.divide(Ideal,Mo,out=np.zeros_like(Ideal), where=Mo!=0)

Idrh3=np.multiply(Idrh2,W_relativo) #nuevo
'''
fig = plt.figure(11)
...
for d in range(0,128):
    Effi_d[d]=np.divide(Ideal1, Idrh2[d],out=np.zeros_like(Ideal1), where=Idrh2[d]!=0)
...
fig = plt.figure(13)
plt.pcolormesh(effi1,clim=(0.5,2))
fig = plt.figure(14)
plt.hist(effi1.ravel(), bins=266, range=(0.01, 4.0), fc='k', ec='k')
fig = plt.figure(15)
plt.imshow(effi1,clim=(0.75,3))
